...
const getUpAllVideosList = async mid => {
	...
	const videosList = [];
	// 获取 up 主制作的视频的数量
	const num = await RequestQueue.reaquest(() => getUpVideosNum(mid));
	...
	while (currentPage <= time) {
		...
		const { code, newList } = await RequestQueue.reaquest(() => getOneGroupUpVideoInfo(mid, currentPage));
		// 如果请求出错，就再来一次
		if (code === -1200) {
			continue;
		}
...
const getGuguData = async mid => {
	...
	console.log(`最大更新时长：${getTimeDiff(maxGuguLength)}`);
};
